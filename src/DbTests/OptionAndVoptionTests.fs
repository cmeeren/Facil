        ...
        testList (nameof DbGen.Procedures.dbo.ProcOptionTableOutWithDto) [
            yield!
                ...
                |> List.map (fun (name, exec) ->
                    ...
                    <| fun () ->
                        let res =
                            DbGen.Procedures.dbo.ProcOptionTableOutWithDto.WithConnection(Config.connStr)
                            |> exec
                            |> Seq.toList

                        let expected: DbGen.TableDtos.dbo.OptionTableWithDto list = [
                            { Col1 = Some "test1"; Col2 = Some 1 }
                            { Col1 = None; Col2 = None }
                        ]
        ...
        testList (nameof DbGen.Procedures.dbo.ProcOptionTableOutWithoutDto) [
            yield!
                ...
                |> List.map (fun (name, exec) ->
                    ...
                    <| fun () ->
                        let res =
                            DbGen.Procedures.dbo.ProcOptionTableOutWithoutDto.WithConnection(Config.connStr)
                            |> exec
                            |> Seq.toList
        ...
        testList (nameof DbGen.Scripts.Option.TableOutWithDto) [
            yield!
                ...
                |> List.map (fun (name, exec) ->
                    ...
                    <| fun () ->
                        let res =
                            DbGen.Scripts.Option.TableOutWithDto.WithConnection(Config.connStr)
                            |> exec
                            |> Seq.toList
        ...
        testList (nameof DbGen.Scripts.Option.TableOutWithoutDto) [
            yield!
                ...
                |> List.map (fun (name, exec) ->
                    ...
                    <| fun () ->
                        let res =
                            DbGen.Scripts.Option.TableOutWithoutDto.WithConnection(Config.connStr)
                            |> exec
                            |> Seq.toList
                            |> List.map (fun r -> {| r with Col1 = r.Col1 |})
        ...
        testList (nameof DbGen.Procedures.dbo.ProcVoptionTableOutWithDto) [
            yield!
                ...
                |> List.map (fun (name, exec) ->
                    ...
                    <| fun () ->
                        let res =
                            DbGen.Procedures.dbo.ProcVoptionTableOutWithDto.WithConnection(Config.connStr)
                            |> exec
                            |> Seq.toList
        ...
        testList (nameof DbGen.Procedures.dbo.ProcVoptionTableOutWithoutDto) [
            yield!
                ...
                |> List.map (fun (name, exec) ->
                    ...
                    <| fun () ->
                        let res =
                            DbGen.Procedures.dbo.ProcVoptionTableOutWithoutDto.WithConnection(Config.connStr)
                            |> exec
                            |> Seq.toList
                            |> List.map (fun r -> {| r with Col1 = r.Col1 |})
                    ...
                    <| fun () ->
                        let res =
                            DbGen
                                ...
                                .WithParameters(
                                    tvp = [
                                        DbGen.TableTypes.dbo.MultiColNullVoption.create (
                                            Foo = ValueSome 1,
                                            Bar = ValueSome "test"
                                        )
                                    ]
        ...
        testList (nameof DbGen.Scripts.Voption.TableOutWithDto) [
            yield!
                ...
                |> List.map (fun (name, exec) ->
                    ...
                    <| fun () ->
                        let res =
                            DbGen.Scripts.Voption.TableOutWithDto.WithConnection(Config.connStr)
                            |> exec
                            |> Seq.toList
        ...
        testList (nameof DbGen.Scripts.Voption.TableOutWithoutDto) [
            yield!
                ...
                |> List.map (fun (name, exec) ->
                    ...
                    <| fun () ->
                        let res =
                            DbGen.Scripts.Voption.TableOutWithoutDto.WithConnection(Config.connStr)
                            |> exec
                            |> Seq.toList
                            |> List.map (fun r -> {| r with Col1 = r.Col1 |})
