...
let tests =
  testSequenced <| testList "Table script tests" [
    ...
    testList "Basic execute + can roundtrip values without losing precision" [
      ...
      testCase "Non-null INSERT/UPDATE batch" <| fun () ->
        Property.check <| property {
          clearTableScriptTables ()
      ...
      testCase "Non-null MERGE batch" <| fun () ->
        Property.check <| property {
          clearTableScriptTables ()
      ...
      testCase "Null INSERT/UPDATE batch" <| fun () ->
        Property.check <| property {
          clearTableScriptTables ()
    ...
    testList "Misc" [
      ...
      testCase "Batch updates only the specified rows" <| fun () ->
        Property.check <| property {
          clearTableScriptTables ()
      ...
      testCase "Identity, output columns and other column config with batch INSERT/UPDATE" <| fun () ->
        Property.check <| property {
          clearTableScriptTables ()
          ...
          let insertRes =
            DbGen.Scripts.TableWithIdentityCol_InsertBatch
              ...
              .WithParameters(
                [
                  DbGen.Scripts.TableWithIdentityCol_InsertBatch.args.create(
                    Foo = 0L,
                    BAR = None
                  )
                ]
      ...
      testCase "Identity, output columns and other column config with MERGE batch" <| fun () ->
        Property.check <| property {
          clearTableScriptTables ()
      ...
      testCase "Batch MERGE WITH (HOLDLOCK) works" <| fun () ->
        Property.check <| property {
          clearTableScriptTables ()
      ...
      testCase "Skips computed columns in batch insert scripts" <| fun () ->
        let f () =
          DbGen.Scripts.TableWithComputedCol_InsertBatch
            .WithConnection(Config.connStr)
            .WithParameters(
              [
                DbGen.Scripts.TableWithComputedCol_InsertBatch.args.create(0, 0L)
              ]
            )
            .Execute()
          |> ignore
      ...
      testCase "Skips computed columns in batch insert scripts even if skip = false" <| fun () ->
        let f () =
      ...
      testCase "Skips computed columns in batch update scripts" <| fun () ->
        let f () =
          DbGen.Scripts.TableWithComputedCol_UpdateBatch
            .WithConnection(Config.connStr)
            .WithParameters(
              [
                DbGen.Scripts.TableWithComputedCol_UpdateBatch.args.create(0, 0L)
              ]
            )
            .Execute()
          |> ignore
      ...
      testCase "Skips computed columns in batch update scripts even if skip = false" <| fun () ->
        let f () =
      ...
      testCase "Skips computed columns in batch merge scripts" <| fun () ->
        let f () =
          DbGen.Scripts.TableWithComputedCol_MergeBatch
            .WithConnection(Config.connStr)
            .WithParameters(
              [
                DbGen.Scripts.TableWithComputedCol_MergeBatch.args.create(0, 0L)
              ]
            )
            .Execute()
          |> ignore
      ...
      testCase "Skips computed columns in batch merge scripts even if skip = false" <| fun () ->
        let f () =
