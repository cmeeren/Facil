...
// Hash: 81b8f9c6755eb3a6af866d68680f4b063f214396e5bba6c7365d6d78608a84ea
...
module Procedures =
  ...
  module ``dbo`` =
    ...
    type ``ProcNominalResult`` private (connStr: string, conn: SqlConnection) =
      ...
      static member WithConnection(connectionString, ?configureConnection: SqlConnection -> unit) =
        ``ProcNominalResult``(connectionString, null).ConfigureConnection(?configureConnection=configureConnection)

      static member WithConnection(connection) = ``ProcNominalResult``(null, connection)
...
module Scripts =
  ...
  type ``NominalResult`` private (connStr: string, conn: SqlConnection) =
    ...
    static member WithConnection(connectionString, ?configureConnection: SqlConnection -> unit) =
      ``NominalResult``(connectionString, null).ConfigureConnection(?configureConnection=configureConnection)

    static member WithConnection(connection) = ``NominalResult``(null, connection)
