...
let rec getInnerEx (ex: exn) =
    if isNull ex.InnerException then
        ...
        getInnerEx ex.InnerException


let (|InnerException|) ex = getInnerEx ex
...
let inline allExecuteMethodsAsSingle< ^a, 'b
    when ^a: (member ExecuteSingle: unit -> 'b option)
    ...
    "ExecuteSingleAsync",
    fun x ->
        (^a: (member ExecuteSingleAsync: _ -> _) (x, None))
        |> Async.AwaitTask
        |> Async.RunSynchronously
    "Execute", (fun x -> (^a: (member Execute: unit -> _) x) |> Seq.tryHead)
    "AsyncExecute",
    fun x ->
        (^a: (member AsyncExecute: unit -> _) x)
        |> Async.RunSynchronously
        |> Seq.tryHead
    "ExecuteAsync",
    fun x ->
        (^a: (member ExecuteAsync: _ -> _) (x, None))
        |> Async.AwaitTask
        |> Async.RunSynchronously
        |> Seq.tryHead
    "AsyncExecuteWithSyncRead",
    fun x ->
        (^a: (member AsyncExecuteWithSyncRead: unit -> _) x)
        |> Async.RunSynchronously
        |> Seq.tryHead
    "ExecuteAsyncWithSyncRead",
    fun x ->
        (^a: (member ExecuteAsyncWithSyncRead: _ -> _) (x, None))
        |> Async.AwaitTask
        |> Async.RunSynchronously
        |> Seq.tryHead
    ...
    "LazyExecuteAsync",
    fun x ->
        (^a: (member LazyExecuteAsync: _ -> _) (x, None))
        |> AsyncSeq.ofAsyncEnum
        |> AsyncSeq.toBlockingSeq
        |> Seq.tryHead
    "LazyExecuteAsyncWithSyncRead",
    fun x ->
        (^a: (member LazyExecuteAsyncWithSyncRead: _ -> _) (x, None))
        |> AsyncSeq.ofAsyncEnum
        |> AsyncSeq.toBlockingSeq
        |> Seq.tryHead
]
...
let inline allExecuteMethodsAsSingleVoption< ^a, 'b
    when ^a: (member ExecuteSingle: unit -> 'b voption)
...
let inline allEagerSingleExecuteMethods< ^a, 'b
    when ^a: (member ExecuteSingle: unit -> 'b)
...
let inline allEagerSeqExecuteMethods< ^a, 'b
    when ^a: (member Execute: unit -> 'b)
    ...
    fun x ->
        ...
        |> Async.RunSynchronously
    "AsyncExecuteWithSyncRead",
    fun x -> (^a: (member AsyncExecuteWithSyncRead: unit -> _) x) |> Async.RunSynchronously
    "ExecuteAsyncWithSyncRead",
    fun x ->
        (^a: (member ExecuteAsyncWithSyncRead: _ -> _) (x, None))
        |> Async.AwaitTask
...
let inline allSeqExecuteMethods< ^a, 'b
    when ^a: (member Execute: unit -> ResizeArray<'b>)
    and ^a: (member AsyncExecute: unit -> Async<ResizeArray<'b>>)
    and ^a: (member ExecuteAsync: CancellationToken option -> Task<ResizeArray<'b>>)
    and ^a: (member AsyncExecuteWithSyncRead: unit -> Async<ResizeArray<'b>>)
    and ^a: (member ExecuteAsyncWithSyncRead: CancellationToken option -> Task<ResizeArray<'b>>)
...
let inline allLazyExecuteMethods< ^a, 'b
    when ^a: (member LazyExecute: unit -> seq<'b>)
    and ^a: (member LazyExecuteAsync: CancellationToken option -> Collections.Generic.IAsyncEnumerable<'b>)
    and ^a: (member LazyExecuteAsyncWithSyncRead: CancellationToken option -> Collections.Generic.IAsyncEnumerable<'b>)> = [
...
let inline allNonResultExecuteMethods< ^a, 'b
    when ^a: (member Execute: unit -> 'b)
    and ^a: (member AsyncExecute: unit -> Async<'b>)
    and ^a: (member ExecuteAsync: CancellationToken option -> Task<'b>)> = [
    "Execute", (fun x -> (^a: (member Execute: unit -> _) x))
...
module Gen =
    ...
    module Sql =
        ...
        let binary n =
            if n < 1 || n > 8000 then
                invalidArg (nameof n) "Must be between 1 and 8000"
        ...
        let datetime2 (n: int) =
            ...
            Range.linearFrom (DateTime(2000, 1, 1)).Ticks DateTime.MinValue.Ticks DateTime.MaxValue.Ticks
            |> Gen.integral
        ...
        let nchar n =
            if n < 1 || n > 4000 then
                invalidArg (nameof n) "Must be between 1 and 4000"
            // Limit to UTC-2 code points to ensure that the storage size is equal to n.
            // Don't include NULL because it seems to mess with test output.
            Gen.choice [ Gen.char '\u0001' '\uD7FF'; Gen.char '\uE000' '\uFFFF' ]
        ...
        let smalldatetime =
            ...
            Range.linearFrom (DateTime(2000, 1, 1)).Ticks (DateTime(1900, 1, 1)).Ticks (DateTime(2079, 6, 6)).Ticks
            |> Gen.integral
            |> Gen.map (adjustTickPrecision >> DateTime)
