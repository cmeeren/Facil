...
type private ManifestConfig = {
    path: string
    configsHash: string
    rulesetsHash: string
}

type private ManifestScript = { path: string; hash: string }
...
let private manifestJsonOptions =
    let options =
...
let renderManifestHeader version assemblyHash yamlFileName configsHash rulesetsHash (scripts: (string * string) list) =
    let manifest = {
        ...
        config = {
            ...
            rulesetsHash = rulesetsHash
        }
        scripts = scripts |> List.map (fun (path, hash) -> { path = path; hash = hash })
    }

    let jsonLines =
        JsonSerializer
            .Serialize(manifest, manifestJsonOptions)
            .ReplaceLineEndings("\n")
            .Split("\n")
...
let renderDocument (cfg: RuleSet) (headerLines: string list) (everything: Everything) =
    [
        yield! headerLines
        ""
        $"[<System.CodeDom.Compiler.GeneratedCode(\"Facil\", \"{(getFacilVersion ())}\")>]"
