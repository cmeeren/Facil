...
let logYamlWarning fullPath lineNo colNo msg =
    logYaml fullPath lineNo colNo "warning" msg

let logYamlError fullPath lineNo colNo msg =
    logYaml fullPath lineNo colNo "error" msg

let failwithError<'a> msg : 'a =
    raise (FacilException(fun () -> logError msg))
...
module String =
    ...
    let trimStart (trim: char) (s: string) = s.TrimStart(trim)
    ...
    let getDeindentedLines (s: string) =
        ...
        let minIndent =
            s |> List.map (fun s -> s |> Seq.takeWhile ((=) ' ') |> Seq.length) |> List.min
